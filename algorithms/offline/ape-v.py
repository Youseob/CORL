...
class VectorizedCritic(nn.Module):
    def __init__(
        self, state_dim: int, action_dim: int, hidden_dim: int, belief_dim: int, num_critics: int
    ):
        super().__init__()
        self.critic = nn.Sequential(
            VectorizedLinear(state_dim + belief_dim + action_dim, hidden_dim, num_critics),
            nn.ReLU(),
...
class APEV:
    def __init__(
        ...
        self.tau = tau
        self.gamma = gamma
        self.agent_mask = torch.tril(torch.ones(self.belief_dim, self.belief_dim + 1) * Q_MAX_VALUE, diagonal=-1).to(self.device)
    ...
    def _get_belief(
        ...
        with torch.no_grad():        
            ...
            # min prob 0.01 --> td.err max 값이 7
            # log_likelihood = torch.clip(td_err, None, 7)
            # likelihood = torch.exp(-td_err) # TODO CHECK CLIP 
        log_next_belief = torch.log(belief) - td_err - torch.logsumexp((-td_err + torch.log(belief)), dim=1, keepdim=True)
        next_belief = torch.exp(log_next_belief) 
        if torch.isnan(belief).any() or torch.isnan(next_belief).any():
    ...
    def _actor_loss(
        ...
        # [ensemble_size, batch_size]
        qs = self.critic(state, belief, action).T
        qs = qs[:, None, :].repeat(1, self.belief_dim, 1) + self.agent_mask
        # [batch_size, belief_dim]
        q_value = qs.min(-1).values
        assert q_value.shape[1] == self.belief_dim
        # [batch_size, ensemble_size] --> batch_size
        filtered_q_value = (belief * q_value).sum(-1)
    ...
    def _critic_loss(
        ...
        with torch.no_grad():
            next_action, log_prob = self.actor(next_state, next_belief, need_log_prob=False)
            # [batch_size, num_critics] 
            qs_next = self.target_critic(next_state, next_belief, next_action).T
            # [batch_size, num_agent, num_critics]
            qs_next = qs_next[:, None, :].repeat(1, self.belief_dim, 1) + self.agent_mask
            # [batch_size, num_agent]
        ...
        # [batch_size, belief_dim]
        q_value = qs.min(-1).values
        # [batch_size, ensemble_size] - [batch_size, ensemble_size]
        
        td_err = ((q_value - q_target) ** 2)
        if need_td_err: return td_err #torch.abs(q_value - q_target)
        loss = td_err.sum(dim=1).mean()
        return loss
...
def train(config: TrainConfig):
    ...
    actor = Actor(state_dim, action_dim, config.hidden_dim, config.num_critics-1, config.max_action)
    actor.to(config.device)
    actor_optimizer = torch.optim.Adam(actor.parameters(), lr=config.actor_learning_rate)
    critic = VectorizedCritic(
        state_dim, action_dim, config.hidden_dim, config.num_critics - 1, config.num_critics
    )
